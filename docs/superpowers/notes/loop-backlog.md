...
- [ ] **Wave 2 — P1 fixes top-down** ← IN PROGRESS. ✅ F-LIVE (`c07cdce`). ✅ F-ERRSILENT (closed-by-architecture). ✅ F-CONTRAST (`275c421`). ✅ rail-adapt (FO-RAIL+BU-RAIL `6cee2a6`; FO-COMB closed-already-honest). **All cross-cutting P1s closed** + ✅ SC-HEAD (`32a11a4`) + ✅ F-A11Y contrast half (`046b440`, token-contrast regression guard). **Cross-cutting P1 work essentially complete** (F-A11Y's keyboard/aria half lives in the surface KEYD items). Next: **surface-specific P1s** — lead with **SA-SEM** (Samples non-semantic table → role=table/row/columnheader/cell + labeled thumbnail buttons; Samples audit 15/20 is the lowest), then FOL-KBD, SC-FIELD, SC-ORDER, BU-DEAD, BU-EXPORT, LO-KEYD, SA-KEYD, SA-RESP. Open follow-ups: SC-FIELD (P1), SC-COLD (P2), FA-SUNK (P2).
