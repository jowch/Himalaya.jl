...
- [ ] **Wave 2 — P1 fixes top-down** ← IN PROGRESS. ✅ F-LIVE (`c07cdce`). Next cross-cutting P1s: **F-ERRSILENT** (remaining: network errors + retry), **F-CONTRAST** (app-wide token, verify 3.16:1 first), the **rail adapt** (BU-RAIL+FO-RAIL+FO-COMB), **SC-HEAD** (region headings). Then surface-specific P1s.

> **⚠ HARNESS STATUS (next wake, read first):** the dev backend (julia :8091) **died mid-session and its DB copy `/tmp/himalaya-uat/himalaya.db` was reclaimed by macOS /tmp cleanup** — so live render-verify is currently DOWN (vite :5182 still up but proxies to a dead :8091 → API 500s). F-LIVE was committed on its green gate (invisible SR change; toast primitive already render-verified) with render-verify waived-with-note. **Before the next render-verify step, reconstruct the live-audit harness** per [docs/frontend-dev-loop.md](../../frontend-dev-loop.md) §"side-by-side": rebuild a writable DB copy under `/tmp/himalaya-uat/` and restart `julia --project=packages/HimalayaUI ... serve --port 8091` (from-source; sysimage stale). The prior data was a small experiment (samples D1 "HEPES Only"/C1 "1-1 Only", a "series by temperature") — locate the source experiment dir or regenerate; see [[feedback_live_audit_harness]]. The rail-adapt iteration is layout/visual and genuinely needs rendering.
...
| F-ERRSILENT | **Failed mutations fail silently** — Focus (`queries.ts:888` error read by nobody), Loupe, Builder; legacy of the cancelled conflict-UI decision. Surface plain-language error + retry. | harden | P1 | todo | ✓W1 · **partially advanced by `c07cdce`** (the queue `onError` toast is now assertive; SeriesBuilder confirm-error toasts). REMAINING: non-validation/network errors surfaced + a retry affordance |
