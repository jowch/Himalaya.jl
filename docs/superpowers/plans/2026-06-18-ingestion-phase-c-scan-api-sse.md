...
@patch "/api/experiments/{id}" function(req::HTTP.Request, id::Int)
    ...
    # No recognized patchable (geometry) field → 400, matching the codebase's
    # PATCH validation convention (cf. PATCH /samples in test_route_validation_routing.jl:
    # a body with no patchable field is a bad request, not a 200 no-op).
