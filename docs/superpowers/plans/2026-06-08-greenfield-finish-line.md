...
### M-F — Deferred polish 🟡 (e2e triage ✅; boneyard/fidelity = optional enhancement)
**e2e triage DONE + confirmed (2026-06-09).** Commits `fe8ac3d` (re-point permalinks + figure-export) · `2badc17` (re-point onboarding-tutorial + delete 3 retired curation/stale-banner smoke specs) · `d6495bd` (quarantine 3 deferred q-link specs). **Final e2e: 35 passed / 3 skipped (q-link fixme'd w/ tracking note) / 0 failed** (was 31/0/10), independently re-confirmed by orchestrator. Dispositions: FIX 4 (permalinks testid `focus-workspace-page`→`focus-workspace`; figure-export ×2 re-pointed to greenfield `ExportButton` chevron-menu + filename assertion to the REAL greenfield contract `-YYYY-MM-DD.{png,svg}`, dropping the retired `himalaya-trace-` prefix without weakening; smoke tutorial `plot-title`→`trace-plate`), QUARANTINE 3 (q-link ring↔row — deferred page wiring), DELETE 3 (smoke curate/reanalyze/round-trip — retired auto-peaks curation + stale-indices banner, no greenfield equivalent; matches the file's own legacy-Index removal precedent; replacement coverage = FocusWorkspacePage.*.test + assignment Vitest). No CI wired (project has no CI, per user). No greenfield component changed (test-only).
- **Boneyard captures + fidelity pass = OPTIONAL ENHANCEMENT, not a merge blocker.** The greenfield pages (folio/loupe/scoping/series-builder) already render skeletons from their inline `fixture={…}` props via `boneyard-js/react`; committed `.bones.json` (currently: contact-sheet/focus/plot-card) are higher-fidelity real-DOM recordings, not required for correctness. All surfaces live-audited this session render correctly. → finish-line scope decision pending with user.
