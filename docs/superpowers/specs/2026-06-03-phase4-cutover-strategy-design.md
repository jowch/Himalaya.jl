...
2. **Assemble the page** — `src/print/pages/<Name>Page.tsx` composes the route's composites into the full body (the shell-level composition the assembly stories deferred), wrapped in `<PageFrame width="…">` (see *Cross-cutting: the layout frame*). Imports NEW + CARRIED only.
...
## Cross-cutting: the layout frame (max-width coherence)

Page-width is enforced **structurally**, not per-page, so no page can sprawl or hand-roll (and forget) its own cap — the failure mode that left the legacy contact sheet uncapped and let the Loupe filmstrip balloon its column. The design deliberately varies width per surface (760 → 1380), so coherence means *one source of truth for the per-surface set*, **not one global number**. Two complementary pieces:

1. **`PageFrame` — single source of truth for per-surface widths (greenfield).** A small `src/print/` layout primitive that centers + pads a page body and takes a named width from one map: `{ loupe: 1100, sheet: 1240, focus: 1160, folio: 1380, scoping: 760, builder: 1180 }` (the mockup values). Greenfield pages wrap their body in `<PageFrame width="…">` instead of a literal `mx-auto max-w-[…]`; changing a surface width is then a one-line edit in one file, and the topbar app-frame reads the same `sheet` value so the chrome stays coherent. The Loupe filmstrip lesson (`minmax(0,1fr)` + `min-w-0` + `justify-center-safe`) is baked into `PageFrame`'s body contract — the frame guarantees its children cannot force it wider than its cap. **Established as the first task of the Focus + Samples plan; the already-shipped Loupe page refactors onto it then** (`mx-auto max-w-[1100px]` → `<PageFrame width="loupe">`), and every greenfield page from Focus onward consumes it.

2. **Shell backstop — a ceiling that catches everything (incl. legacy mid-migration).** A `max-w-[1380px] mx-auto` wrapper around the `CorpusShell` `<Outlet>` (1380 = the widest surface, so it never fights a page's own tighter cap — purely a no-sprawl ceiling). This de-sprawls any page lacking its own cap, **including legacy bodies still present during the mixed-state window**, without editing legacy code. **Folded into the shell reskin (build step 8).**
...
2. **Focus** (`/sample/:sampleId`) — fail-fast on the riskiest integration: the TracePlot **Observable Plot → d3** engine swap + the durable assignment model. **Plan opener: establish `PageFrame`** (the layout-frame primitive + width map) and refactor the shipped Loupe page onto it (see *Cross-cutting: the layout frame*). Gaps: `StaleIndicesBanner` (hash-stale debounce), `SpeculativeBuilder` (anchor + ratio dialog), `FocusNotesMargin` (q-ref notes + focus-gated textarea + drawer), `losingPeakIds` dim, q-link/`onHoverQ` hover circuit.
...
8. **Shell reskin** — swap `CorpusShell` / `CorpusTopbar` / `NavModal` / `OnboardingFlow` / `InfrastructureBanner` / toast container onto print primitives; wire `CorpusTopbar onChange → navigate` (derive active from pathname); **add the shell layout backstop** (`max-w-[1380px] mx-auto` around the `<Outlet>` — see *Cross-cutting: the layout frame*). Delete the remaining OLD shell. Full-suite gate. The app is now entirely greenfield.
...
- Every greenfield page body is wrapped in `PageFrame` (no hand-rolled `max-w-[…]` page literals); the `CorpusShell` backstop caps the `<Outlet>` at 1380. Width is centered and sprawl-free at every viewport.
