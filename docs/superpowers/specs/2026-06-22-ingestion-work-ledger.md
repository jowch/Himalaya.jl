...
- **(→ 8c, OPEN)** Scanning UI does not auto-flip to "Confirm groups" on scan completion; needs a manual reload. The `/api/events` 500s were benign `EPIPE` (client disconnect on reload), so the SSE stream isn't dead — likely a completion-frame race or the experiment-query invalidation not refetching `ingest_status`. Defense-in-depth: a short `refetchInterval` on `ingest_status` while scanning. *Needs repro + decision.*
