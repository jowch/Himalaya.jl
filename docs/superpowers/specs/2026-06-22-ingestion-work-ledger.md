...
- [x] **8c. P2 — scanning UI auto-complete** (`95ef696f`) — root cause was a stale live overlay: the at-most-once `/api/events` stream drops the terminal `ingest_complete` frame on an EventSource reconnect (benign EPIPE), leaving `ingestInFlight` stuck `"scanning"`. Fixed with one shared `effectiveIngestStatus` rule (rescan overlay wins; else terminal persisted overrides a stale scanning overlay) + a scoped `useExperiment` `refetchInterval` that self-heals the persisted status while a scan is active. Wired into GroupingReviewPage / ExperimentShell / ExperimentCorpusPage. Live-verified on a fresh DB: scan completes → grouping auto-flips to review + enabled Confirm, no reload.
...
- [ ] **9a. P2 — `serve` fatally errors when the DB file is missing** (`cli.jl:261` `error("no database … — ingest via the HTTP scan API first")`). A brand-new user running `serve` on a fresh box gets a fatal error instead of the empty experiments home, which contradicts the "dump a dir → it figures it out" goal (you can't reach the HTTP scan API without a running server, and the server won't start without a DB — a bootstrap deadlock). Fix candidate: have `serve` create+migrate an empty DB via `open_db` when the path is absent (the very next line already calls `open_db`). Tension: the error also guards a typo'd `HIMALAYA_DB_PATH` in prod — decide guard-vs-bootstrap (maybe gate on a `--init`/empty-dir heuristic). *Needs a decision.*
- [ ] **9b. P2 — onboarding welcome tour copy is STALE.** The 4-step `OnboardingFlow` tour ("Three stages, one corpus" / "Triage: screen and choose" / …) describes the retired *"Triage, Index, Series stage tabs run across the top"* navigation model. The current TopNav is `Experiments | Series` with no stage tabs. The copy must be rewritten for the corpus/funnel IA (or the tour retired). *Pure copy + 4 strings; no anchor in the canonical spec.*
- [ ] **9c. P2 — `Input` is not a `forwardRef` component, so `Popover` can't attach its anchor ref** (console: "Function components cannot be given refs" at `DirectoryPickerField → Popover → Input`, `Input.tsx`). Non-fatal (path validation still worked) but the suggestion popover may mis-anchor, and it's a recurring console error on every funnel visit. Fix: wrap `Input` in `React.forwardRef` (closed-look primitive; placement ref only).
