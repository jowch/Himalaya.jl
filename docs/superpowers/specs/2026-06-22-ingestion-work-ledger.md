...
- [ ] **6c-2. Re-arm rescan schedulers on server boot** — the per-experiment rescan timers are in-memory (`start_rescan_scheduler!`), lost on restart; nothing re-arms them at startup (server.jl comment even says "re-armed by the first scan or manual rescan"). A boot hook should re-arm schedulers for all `complete` experiments. *Backend, small; needs a startup-hook decision.*
