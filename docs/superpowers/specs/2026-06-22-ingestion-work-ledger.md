...
- [x] **10. Geometry table formatting** — ragged columns (only label had fixed width); value→fixed-width column + right-aligned actions so chips/Override align; um→µm. DONE `b204a52a`.
- [x] **11. Source chips unified** — extracted shared `SourceChip` (label+emphasized); used in BOTH GeometryLedger (gear) and the funnel (was generic Chip). DONE `b204a52a`.
- [x] **12. Acquisition date overlap** — `AcquisitionChart` MIN_SLOT per session cluster (bars centered) so adjacent centered date labels can't collide. DONE `b204a52a`.
...
**Grouping-review live-walk batch (2026-06-23, `a0b2c1df`) — 4 fixes (frontend-reviewer CLEAN):**
- [x] **Live unfold was dead (empty until manual refresh).** Loads/samples commit in ONE transaction before any `ingest_progress` frame fires, so the surface relied on those SSE frames invalidating the loads query — but React Query ABSORBS an invalidation that lands during an in-flight fetch (the frame burst overlaps the first empty mid-scan fetch), and `staleTime` (30s) then keeps the empty result "fresh" so nothing re-fetches. The 8c self-heal polled the experiment row, never loads. Fix: `useLoads(id, refetchWhileScanning)` polls every 2s while scanning (mirrors `useExperiment`'s ingest self-heal); `GroupingReviewPage` computes `scanning` before `useLoads(experimentId, scanning)`. SSE stays the fast path; the poll is the un-raceable floor. **Live-verified: forced `ingest_status='scanning'` on a real experiment → loads polled at 2009/2009/2013ms cadence + rendered while scanning; single fetch when complete.** LESSON: invalidation-during-inflight-fetch is silently absorbed; a poll is the robust floor for live-derived surfaces.
- [x] **Cull (bulk-merge) bar covered by the Confirm footer.** `GroupingBulkBar` was `bottom-6` z-auto under the footer (`bottom-0 z-30`). Lifted to `bottom-20 z-40`. Live-verified: bar bottom 1024 <= footer top 1053.
- [x] **Rename = far-right button.** Replaced with a pencil `IconButton` next to the sample name (funnel idiom); shrank the inline title input display->headline via new `inputSize="sm"` on `Input`'s title variant (default `md` keeps display, so funnel/scoping/shell title consumers are unaffected).
- [x] **Flag dismiss undiscoverable + ABSENT for split flags** (only reshoot/merge had "Keep separate", buried in the expanded body). Added a dismiss (x) `IconButton` on the flag chip itself for BOTH kinds (backend dismiss route already handled both); pulled the chip out of the name `<button>` to kill a nested-interactive.
