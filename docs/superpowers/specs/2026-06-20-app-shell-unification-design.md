...
- Focus + Loupe stay **flat leaf routes** — `/sample/:id` (Focus) and `/sample/:id/loupe` (Loupe). They are genuinely cross-cutting (reached from the corpus *and* from cross-experiment Series), and a sample's id is globally unique, so a flat address is honest and avoids a fetch-then-redirect that would always flash a loading screen. The experiment context the chrome needs (header, `‹ Corpus` up-link) is resolved from the loaded sample's `experiment_id` — which the page must fetch to render anyway — so nothing extra is loaded. (Rev 2: this supersedes the earlier "experiment-scoped Focus/Loupe" wording.)
- `/series` (folio) + scoping/builder — cross-experiment; reached from the top `Series` section and via "send to series" from a corpus selection. A Series member opens Focus carrying a `from=series` return marker (query param / router state), so the up-link reads `‹ Series` (rather than `‹ Corpus`) for that visit — this marker, not the path shape, resolves the back-target ambiguity (the path can't encode *which* parent you came from either way).
