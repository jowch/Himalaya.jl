...
4. **Extend `GET /api/sample-tags`** (lives in **`routes_picker.jl`**, not routes_samples.jl) to return a per-`(key,value)` **sample count**: rewrite its `SELECT DISTINCT key, value` to `SELECT key, value, COUNT(DISTINCT sample_id) AS count … GROUP BY key, value`. Decide whether the experiment-scoped sibling `GET /api/experiments/{eid}/sample-tags` (same `DISTINCT` contract) gets the same treatment (recommended: yes, for symmetry). The `count` field is additive on the wire, **but** it is **not** behavior-neutral for the one existing consumer: `proposeOrdering.ts:36` currently ranks ordering keys by *number of distinct `(key,value)` pairs*, not sample count. Wiring the suggestion counts means `proposeOrdering` should rank by **summed sample count** instead — a deliberate improvement (sample-count ranking is the correct frequency signal) that **will move the proposed ordering variable on some corpora**. Treat it as an intentional change with its own test, not a silent additive field.
...
3. **Counted `GET /api/sample-tags`** (both the corpus + experiment routes in `routes_picker.jl`) + the intentional `proposeOrdering` sample-count ranking change (with its test).
