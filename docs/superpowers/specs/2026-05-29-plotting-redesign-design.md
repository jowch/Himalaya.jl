...
**The single smart automation: the Gauss–Bonnet ratio.** Once a bicontinuous cubic (Pn3m / Im3m / Ia3d) is in the assignment, the lattice parameter of a coexisting cubic is *predicted* by the Bonnet ratio. Candidates whose measured `a` matches get a **`⭙ Bonnet` flag** (and may sort up *within the candidate-list view only*). This is the principled "in context with the phase I already chose" help — physics, not peak-coverage heuristics. **The flag never touches `score()`** (which stays coverage×consistency) and is never persisted — a score bump would silently reorder `auto_group`/`remove_subsets`, the exact hazard `docs/scoring.md` warns against. Reconciled with Plan B 2026-05-30 (saxs-physics-reviewer). **Bonnet is the only such automation in scope**; the design leaves room for more exact constraints later but commits to none.
...
- **Ranked candidates** with the `⭙ Bonnet` flag (display + optional in-view sort only, never a `score()` bump) on the coexisting cubic whose lattice matches the Gauss–Bonnet ratio.
