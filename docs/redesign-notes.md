...
**Decided — the functional-traversal audit and the implementation program (2026-05-29):**
After milestone #4 closed, a fourth visual audit (`docs/2026-05-29-the-print-round4-findings.md`) and the standing spec-conformance ledger (`docs/2026-05-27-redesign-gap-report.md`) both reported the redesign substantially shipped. Neither could see the thing that turned out to matter most, because both are blind to it by construction: a visual audit reads pixels on a surface it can already reach, and a spec-conformance audit checks the edges someone thought to specify. Reachability — whether you can *get to* a surface by clicking — was never an acceptance criterion of either. A third instrument was run to close the gap: a live functional traversal, clicking every control across all nine surfaces (`docs/2026-05-29-functional-interaction-sweep.md`, with the 161-row inventory in `docs/2026-05-29-functional-sweep-inventory.json`).

- *Headline finding — the indexing workspace is unreachable by clicking.* `/sample/:id` (the focus workspace, the surface where a scientist actually indexes one sample) has no door from the corpus: the sample name / `#id` / status chip are inert spans, a thumbnail double-click opens the loupe (never focus), the loupe offers no path onward ("carries forward to the Index stage" is unlinked text), `NavModal.commitSample` sets the store but never `navigate()`s, and the INDEX nav tab is disabled. Root cause: master-plan Phase 5 retired the old dual-nav door *before* the new representative-exposure door was hung, and no instrument was watching the doorway. The other findings are real but secondary: controls that lie (a no-op Confirm-series button, a cosmetic "+ Peak" mode, a fake Cross-experiment filter / "Variable" sort / "Ordered by" dropdown / drag grips) and orphaned code (the chat subsystem, `TitleButton`, stale onboarding copy).

- *The fix — one room, two doors.* Both entry paths land in the **same** focus workspace on a representative exposure; they differ only in whether the representative is set on the way in. This is the user's call that "it depends on the scientist, so both should be possible" — triage → index (via the loupe's "Set as representative" → carry forward) and corpus → index directly are both valid working paths, not two indexing UIs. The corpus-row door is the **status cell** ("Not indexed" → go index; a phase chip once called → see / adjust), deliberately the new representative-bridge mechanic and *not* a resurrected old Index link — the sample name stays identity. The loupe's "carries forward" text becomes a real link; `NavModal.commitSample` actually navigates. The load-bearing data-shape fix underneath all of them: **guarantee a default `indexing_exposure_id`** (first-accepted / strongest-signal heuristic, overridable in place via the existing focus rep-switcher and the loupe "Set as representative") so the door never opens onto a 404.

- *Depart, do not recover.* The governing principle for the program (the user's): the visual redesign was already a clean split, and shoehorning the old mechanics back is self-defeating. Findings sort into three buckets — **finish the departure** (build the new on-ramp that Phase 5's cutover left unbuilt), **retire lingering old mechanics** (the chat), and **one genuine redesign** (the trace-editing grammar). Recovering what the dark app used to have is explicitly not the move.

- *The chat is retired, not revived.* During the redesign the chat was de-emphasized (low activity for a large share of screen) in favour of the notes margin, and the @-mention chip vocabulary was never actually designed. The user was reluctant to leave it behind but chose to kill it now and reconsider reviving it later. The cut follows the two natural layers. **Gutted (presentation):** `ChatCard`, `MentionChip`, `MentionCompose`, `MentionPicker`, `useMentionResolution`, and `renderMentions` — a closed dead island (every file reachable only through `ChatCard`, which is mounted nowhere) — plus their six dedicated tests. **Parked (data plane + backend, left inert but contract-tested):** the queue `post_message` mutators (`mutators/trivial.ts`), the `post_message` SSE reconciliation (`applyRemoteToCache.ts`), the `SampleMessage` / `ComparisonMessage` types, `queryKeys.messages` / `comparisonMessages`, the `api.ts` client methods (`postSampleMessage` / `postComparisonMessage`), and the Julia backend (`routes_messages.jl`; the `sample_messages` / `comparison_messages` / `series_messages` tables in `db.jl`). Parking the whole vertical data plane together means a future revival re-mounts a compose box rather than re-deriving the message protocol, and the contract tests keep the protocol honest in the meantime. `comparisonHash8` (in the load-bearing `contentHash.ts`) is *retained* despite losing its only callers: it is the documented canonical fix for the #62 mention-token drift bug (`labels.ts` references it as such), and deleting it would risk silently reintroducing that bug on revival. The whole subsystem is recoverable via git regardless.

- *The program — four movements.* (1) **Hang the doors** — the on-ramp above plus the default-`indexing_exposure_id` guarantee. (2) **Stop the controls lying** — wire or hide the no-op Confirm-series button, the cosmetic "+ Peak" mode, and the fake Cross-experiment filter / "Variable" sort / "Ordered by" dropdown / drag grips. (3) **Retire orphans** — the chat (above), `TitleButton`, stale onboarding copy; rename `MultiTracePlot` off its Compare-era identity. (4) **Redesign the trace grammar** — the one genuine redesign: a real "+ Peak" mode with a crosshair, peak-move, a disambiguated triangle-click, and reconciled resets; gets an impeccable `shape` pass before any code. Execution rhythm per movement: a focused branch, TDD where there is testable behaviour, **the live functional traversal as the new acceptance test** (the lens that caught the gap becomes the gate that closes it), and paired review via the project reviewers. Movements 1–3 are wire / delete and go straight to TDD; Movement 4 is shaped before it is built.

**Decided — figure export model + multiplayer conflict stance (2026-06-03):**
The greenfield "The Print" rebuild's deferred component list named two modals with no mockup — `ExportSheet` and `ConflictModal`. A brainstorm collapsed both, and neither survives as a modal. Full design: [docs/superpowers/specs/2026-06-03-figure-export-design.md](superpowers/specs/2026-06-03-figure-export-design.md).

- *Figure export is content-WYSIWYG, style-transformed — and there is no sheet.* What you compose (which traces, peaks, offsets, order) is exactly what exports, but the decorative in-app Print styling is shed for clean scientific presentation (white ground, Arial, heavier strokes) — the look a paper or slide expects, and the look the engine's existing `CLEAN_SCIENTIFIC` preset already renders. There is **no export-time tuning, no preview, no options dialog**: the export is fully determined by the composition. The interaction is a **split button** — primary **Copy** short-circuits straight to the clipboard (one click → clean-render PNG), a chevron reveals **Download as PNG / SVG**. This is the legacy `FigureExportControls` ergonomics reskinned into The Print, *not* the full-screen "ExportSheet" dialog the early roadmap imagined. It ships as two units on the Print presentational contract: a `useFigureExport` hook (the sole consumer of `lib/figure-export/*`) and a presentational `ExportButton`. The clean-render styling stays the engine + per-figure adapter's job (`CLEAN_SCIENTIFIC` + the `ExportSpec`), realized at Layer-4 wire time — the control is style-agnostic.

- *Conflicts are cancelled, not deferred.* The git-style conflict-resolution model — `SeriesCommitConflictModal` / `ConflictModalShell`, server-vs-draft panels, Discard/Overwrite, driven by a `409` + `expected_content_hash` (Plan 7's R5b, framed as *deferred-pending-instrumentation* in [event-log.md](event-log.md)) — is **abandoned for the rebuild**. The user's stance: optimistic-concurrency conflict UI is unnecessary friction. Multiplayer stays last-write-wins (extending the existing deliberate LWW grain on `exposures.selected`), and the positive replacement is **edit-tracking → undo/redo → versioning**, designed for real during Layer 4 where the commit path actually lives (it touches the commit contract, not just the UI). This is a separate Layer-4 sub-project, registered under [future-feature-ideas.md](future-feature-ideas.md) §"Multi-user / review". No conflict modal is built now or revived later in its `409`-retry form.
