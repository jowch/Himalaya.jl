...
|---|---|---|---|---|
| `TracePlate` | Focus hero panel (lifted plate: header + toolbar + trace) — **home of the consumer-owned scale toggle + armed +Peak** — `components/TracePlate.tsx` (Batch 7). Legend lives in `CombsPanel`, not here (per mockup `.combs-legend`). | `Card`(elevated)+`PlateHeader`+`ToolBar`+`TracePlot` | focus-plot | ✅ |
| `DetectorPanel` | detector frame (`bg-frame-edge` aspect-square) + header exposure strip + hint — `components/DetectorPanel.tsx` (Batch 7). Presentational: `src` + exposure-switcher passed in. | `Card`+`PanelHeader`+`DetectorImage`(full)+`ThumbnailGallery`(xs) | focus-plot | ✅ |
