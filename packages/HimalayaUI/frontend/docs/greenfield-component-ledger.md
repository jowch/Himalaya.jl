...
- **Layer 3 tier-2 panels:** ~16 ✅ (Batch 5: `SeriesCard`/`Gallery`; Batch 6: `AssignmentCart` + `CandidateList`; Batch 7: `TracePlate`/`DetectorPanel`/`CombsPanel` + `PanelHeader`; **Batch 8: `AssignmentRail` + `RailSection` + the 6 modal leaves** — `ModalHead`/`ModalFieldRow`/`ModalFooter`/`FitMetadata`/`LatticeParamControl` + the `CustomIndexModal` cluster) · ~6 ⬜ (`SheetTable`, `BigFrame`, `LoupeSidePanel`, `ScopePlate`+scoping `SampleRow`, `MemberList`, `SeriesPlate`, `ReadingPanel`, `BuilderRail`)
- **Modals/overlays:** 1 cluster ✅ (`CustomIndexModal` + 6 leaves, Batch 8; `ModalShell` chrome ✅) · ~5 gap clusters ⬜ (`ExportSheet`, `CullBar`, `RailBack`, `Dock`, `ConflictModal`)
- **Layer 4 pages:** 0/5 ⬜
