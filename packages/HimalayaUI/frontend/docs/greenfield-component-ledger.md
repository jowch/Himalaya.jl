...
- **Layer 3 tier-2 panels:** ~19 ✅ (Batch 5: `SeriesCard`/`Gallery`; Batch 6: `AssignmentCart` + `CandidateList`; Batch 7: `TracePlate`/`DetectorPanel`/`CombsPanel` + `PanelHeader`; Batch 8: `AssignmentRail` + `RailSection` + the 6 modal leaves; **Batch 9: `MemberList` + `ReadingPanel` + `SeriesPlate`**) · ~4 ⬜ (`SheetTable`, `BigFrame`, `LoupeSidePanel`, `ScopePlate`+scoping `SampleRow`, `BuilderRail`)
