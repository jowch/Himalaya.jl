...
|---|---|---|---|
| `Field` (label + value + chevron dropdown affordance) | series rail "Ordering variable" / scoping `OrderField` | new small primitive, or compose `Menu` trigger | ⬜ decide on contact (Batch 13). NOTE: scoping's order-field was built INLINE in `ScopePlate` (a presentational `data-testid="order-field"` button), so the standalone `Field` primitive is still owed by the series-**builder** rail. |
...
- **Layer 0 primitives:** 43 ✅ (foundation complete; +`Swatch` +`PhaseLabel`, Batch 3; +`NoticePill`, Batch 5; **+`FlagButton`, Batch 12**; `Button` gained a uniform `disabled:` treatment, Batch 12)
...
- ~~loupe panels (`BigFrame`, `LoupeSidePanel`)~~ ✅ **DONE (Batch 10)** · ~~scoping (`ScopePlate`+`ScopeSampleRow`+`ScopeCandidateRow`)~~ ✅ **DONE (Batch 12)** · `BuilderRail` (last remaining L3 panel)
