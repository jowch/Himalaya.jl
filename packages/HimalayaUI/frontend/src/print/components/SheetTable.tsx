 ...
 *
 * Narrow viewports (WCAG 1.4.10): the table's intrinsic min-width (~1018px +
 * the CHECKBOX_TRACK_PX checkbox track) exceeds a 1024px viewport, so the
 * header rowgroup AND the rows live together in ONE shared `overflow-x-auto`
 * scroller (`data-testid="sheet-scroll"`) — a narrow viewport SCROLLS rather
 * than clipping columns (Carbon/Polaris). The alignment invariant therefore
 * also depends on the shared scroller: header and rows must scroll as one. The
 ...
 * the table's MIN-content width is exactly the sum of the fixed track minimums
 ...
 * (table > rowgroup > row > columnheader/cell) with no role-bearing wrapper in
 * between. The empty slot renders OUTSIDE the table role tree (a rowgroup may
 * only own rows) and outside the scroller so it spans the Card width.
 *
  ...
  return (
    ...
    >
      {/* ONE shared horizontal scroller for header + rows (alignment invariant).
          ...
          overflows and would otherwise become a Tab stop. */}
            ...
            >
              ...
              {onSort ? (
                ...
                <>
                  ...
                  <Kicker tone="soft" role="columnheader" className="px-4 py-2.5">Phase</Kicker>
                </>
              )}
            </div>
          </div>
