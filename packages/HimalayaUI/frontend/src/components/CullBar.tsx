  ...
  return (
    <div
      ...
      className="fixed bottom-7 left-1/2 z-30 flex -translate-x-1/2 items-center
                 gap-1 rounded-md bg-ink py-[7px] pl-4 pr-2 text-paper
                 shadow-[0_18px_40px_-14px_rgba(40,33,24,.5)]"
    >
      <span className="mr-2.5 text-base font-semibold">
      ...
      <button
        ...
        onClick={onReject}
        className="rounded-md bg-print-accent px-3 py-[7px] text-xs font-semibold
                   text-paper hover:opacity-90"
      >
        Drop
        <span className="ml-1 font-mono text-xs opacity-60">X</span>
      </button>
      <button
        type="button"
        data-testid="batch-clear"
        onClick={onClear}
        className="rounded-md px-3 py-[7px] text-xs font-semibold text-paper/70
                   hover:text-paper"
      >
        Clear
