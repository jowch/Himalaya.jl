  ...
  <div className="flex-1 overflow-hidden">
    <div className="py-2">
      <div className="px-4 pb-1 flex items-baseline gap-2">
        ...
        <span className="text-xs text-fg-muted">{FIXTURE_ROWS.length}</span>
      </div>
...
interface Props {
  ...
  /**
   * Fired on each toggle / override radio selection. The caller commits the
   * pick directly (e.g. `addMember`); the body owns no selection state.
   */
...
export function ComparisonPickerBody({
  experimentId, onPick, alreadyAddedExposureIds,
  searchInputRef,
}: Props): JSX.Element {
  ...
  // Pick-set lookup. SamplePickerRow's checkbox + override radios reflect
  // the actual committed draft state, derived from alreadyAddedExposureIds
  // × picker data.
  const pickedSampleIds = useMemo(() => {
    ...
    return out;
  }, [allRows, alreadyAddedExposureIds]);

  const overrideBySampleId = useMemo(() => {
    // A member exists in the draft for some exposure of this sample. If that
    // exposure is NOT the indexing one, surface as an override so the
    // corresponding radio shows checked.
        ...
        {recentSamples.length > 0 && (
          ...
          >
            ...
            <ul role="listbox" aria-label="Recently used samples" className="flex flex-col">
              {recentSamples.map((r) => {
                // alreadyAdded = "any exposure of this sample is in the
                // draft" — the indexing-only check would miss override-added
                // rows: an un-check of an override-added row would silently
                // no-op since togglePickFor(_, false) does nothing.
