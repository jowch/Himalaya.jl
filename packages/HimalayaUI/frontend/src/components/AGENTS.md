...
- **Shell + chrome**: `CorpusShell` (the single layout shell), `CorpusTopbar` (wordmark + 3 stage-tabs + Beamtime chip — matches the mockup, no utilities), `BandResizeDivider`, `AnnotationToggles`. (I5.1 #182 retired the dual-nav `AppShell` / `AppHeader` / `TabRocker` / `WorkspaceGrid` / `UtilityCluster`; `TitleButton` — the experiment/sample picker title — was retired 2026-05-29 with the chat orphans, as a relic of the experiment-container model; the theme-toggle button + switch-user avatar are deferred to a follow-up issue — the `T` theme shortcut still works.)
...
- **Primitives**: `components/ui/` — **closed-look** design-system primitives (Button, Card, SegmentedControl, PhaseChip, PhaseStrip, ModalShell, Kicker, IconButton, ScoreBar, Dot, ToastContainer, HintText). The primitive owns its appearance via semantic props (`variant`/`size`/`tone`/domain props); a consumer's `className` is **placement-only**. Inline appearance utilities (`text-[…]`, `rounded-[…]`, raw colours, side-stripes) *outside* `ui/` fail the `lint:design` build guard (`scripts/check-design.mjs`). Full contract: [../AGENTS.md](../AGENTS.md) §"Design system"; visual reference `docs/design-system.html`; system `DESIGN.md` (root).
