...
describe("KbdLegend (registry-driven)", () => {
  it("renders each id's key cap and description straight from the registry", () => {
    render(<KbdLegend ids={["drop", "representative"]} />);
    ...
    expect(within(legend).getByText(SHORTCUTS.representative.label)).toBeInTheDocument();
    ...
    expect(caps).toContain(shortcutLabel("representative"));
  });
