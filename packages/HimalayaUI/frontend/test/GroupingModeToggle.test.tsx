 ...
 *
 * Behaviour:
 *   - Reflects the `value` prop on the container's `data-mode`.
 *   - Click dispatches `onChange` with the selected value.
 *   - Three options render: "By sample" / "By phase" / "Distinct".
 *
 * C-4: The component is now prop-driven (value + onChange) rather than
 * reading/writing Zustand directly. The parent resolves the effective mode
 * via effectiveGroupingMode(draft, comparison). The `value`/`onChange`
 * shape unifies with the sibling segmented controls (PR #251 round 1).
 */
...
describe("GroupingModeToggle", () => {
  it("renders the testid + active-mode reflection", () => {
    render(<GroupingModeToggle value="bySample" onChange={() => {}} />);
  ...
  it("clicking 'By phase' calls onChange with 'byPhase'", () => {
    ...
    render(<GroupingModeToggle value="bySample" onChange={onChange} />);
  ...
  it("active option carries data-active='true' and aria-checked='true'", () => {
    render(<GroupingModeToggle value="byPhase" onChange={() => {}} />);
  ...
  it("reflects value='distinct' on container data-mode", () => {
    render(<GroupingModeToggle value="distinct" onChange={() => {}} />);
