...
- **Ingest progress frames carry TWO orthogonal discriminators — `phase` and `stage` — do not conflate them.** `phase` (set by the ROUTE: absent on create, `"rescan"` on `POST /:id/scan`) selects the frontend SURFACE: absent → the `GroupingReviewPage` takeover, `"rescan"` → the inline bar on the corpus page. `stage` (set by `scan_and_group!`: `"discovery"` / `"analyzing"` / `"thumbnails"`) selects which SEGMENT of the progress bar is filling. Overloading `status`/`phase` with the stage would flip the whole surface mid-scan. `on_progress` is `(processed, total, stage)` and each stage reports against its OWN denominator — the bar is segmented (`frontend/src/lib/ingestStages.ts`), so stages no longer share one scale. A frame with no `stage` must keep the pre-segment behavior (plain bar, and the loads/samples invalidation still fires).

- **`scan_directory`'s per-file loop is the expensive half of an ingest, not `readdir`.** Per exposure it does two `isfile` stats plus a full `parse_prp` — three round trips × N over SMB. It reports `on_progress(i, n, "discovery")` because it used to run entirely silent, leaving the UI with no denominator until it finished. Related: `derive_geometry` has a **pre-parsed** method taking `parse_prp` NamedTuples; the ingest paths pass `m.prp` from the metas so the PRPs are not read a second time (the path-based method re-parsed all of them — a second full pass per ingest). Only the funnel preview (`routes_fs.jl`), which has paths and no metas, uses the path-based method.
