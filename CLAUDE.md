...
**Default pair (single dev session, no live prod on the box).** Two terminals:
...
**Side-by-side pair (live prod already on :8080, or two worktrees in parallel).** Use non-default ports + a DB copy. Production data must NEVER be a dev backend's `HIMALAYA_DB_PATH` — copy it first:
...
**Stopping cleanly.** Backend and Vite are both detached background processes; kill by listening port. `pkill -f "port <N>"` is exit-144-noisy in some shells but the kill itself succeeds — verify with `ss -tlnp | grep <port>`:
