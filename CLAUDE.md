...
# `develop` the local core, don't rely on a bare instantiate: Manifest.toml is
# gitignored and there is no [sources], so instantiate resolves Himalaya from the
# REGISTRY — the published core, not this checkout. `[compat] Himalaya = "0.6"`
# only catches a mismatch across VERSIONS; edit phaseratios without bumping and
# registry-vs-local diverge silently again, which is wrong physics, not a load error.
